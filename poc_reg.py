...
def pocfunc(ref_chip, cmp_chip):
    # Windowing to reduce boundary effects
    hanning_window_x = np.hanning(ref_chip.shape[0])
    hanning_window_y = np.hanning(ref_chip.shape[1])
    hanning_window_2d = hanning_window_x.reshape(hanning_window_x.shape[0], 1) * hanning_window_y
    ref_chip, cmp_chip = [ref_chip, cmp_chip] * hanning_window_2d
    F = scipy.fftpack.fft2(ref_chip)
    G = scipy.fftpack.fft2(cmp_chip)
    G_ = np.conj(G)
    ...
    M = np.floor([ref_chip.shape[0] / 2.0, ref_chip.shape[1] / 2.0])  # M = [M1, M2]
    U = M / 2.0  # U = [U1, U2]
    low_pass_filter = np.ones([int(M[0]) + 1, int(M[1]) + 1])
    low_pass_filter = zero_padding(low_pass_filter, ref_chip.shape, U)
    R = R * low_pass_filter
    R = scipy.fftpack.fftshift(R)
...
def main_poc_reg(ref_chip, cmp_chip):
    # calculate phase-only correlation
    poc = pocfunc(ref_chip, cmp_chip)
    # get peak position peak
    max_pos = np.argmax(poc)
    peak = np.array([max_pos / ref_chip.shape[1], max_pos % ref_chip.shape[1]]).astype(int)
    ...
    if fitting_area.shape != (FITTING_SHAPE[0] + 1, FITTING_SHAPE[1] + 1):
        return 0.0, 0.0, 0.0
    m = np.array([ref_chip.shape[0] / 2.0, ref_chip.shape[1] / 2.0])
    u = (m / 2)
